...
class FlipBatchIterator(BatchIterator):
    ...
    def transform(self, X1, X2):
        ...
        bs = X1b.shape[0]
        h_indices = np.random.choice(
            bs, bs / 2, replace=False)  # horizontal flip
        v_indices = np.random.choice(
            bs, bs / 2, replace=False)  # vertical flip

        ###  uncomment these lines if you want to include rotations (images must be square)  ###
        # r_indices = np.random.choice(bs, bs / 2, replace=False) # 90 degree
        # rotation
        for X in (X1b, X2b):
...
# f.close()
# X, y = train_set
# X = np.rint(X * 256).astype(np.int).reshape((-1, 1, 64, 64))  # convert to (0,255) int range (we'll do our own scaling)
...
ae = NeuralNet(
    ...
    hidden_num_units=deconv_filters * (64 + filter_sizes - 1) ** 2 / 4,
    unflatten_shape=(
        ([0], deconv_filters, (64 + filter_sizes - 1) / 2, (64 + filter_sizes - 1) / 2)),
    unpool_ds=(2, 2),
    ...
    batch_iterator_train=FlipBatchIterator(batch_size=500),
    regression=True,
    max_epochs=epochs,
    verbose=1,
)
...
# if your GPU not fast enough, reduce the number of filters in the
# conv/deconv step
...
def get_picture_array(X, index):
    array = X[index].reshape(64, 64)
    #array = np.clip(array, a_min = 0, a_max = 255)
    return array.repeat(4, axis=0).repeat(4, axis=1).astype(np.float32())


def get_random_images():
    index = np.random.randint(30)
    print index
    original_image = Image.fromarray(get_picture_array(X_train, index))
    new_size = (original_image.size[0] * 2, original_image.size[1])
    new_im = Image.new('L', new_size)
    new_im.paste(original_image, (0, 0))
    rec_image = Image.fromarray(get_picture_array(X_train_pred, index))
...
# we find the encode layer from our ae, and use it to define an encoding
# function
...
# N.B after we do this, we won't be able to use the original autoencoder ,
# as the layers are broken up
