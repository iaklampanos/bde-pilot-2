...
def cdetections(date, pollutant, metric, origin):
    ...
    task = cdetections.apply_async(args=[cur, models, lat_lon, date, pollutant, metric, origin])
    return task
...
def detections(date, pollutant, metric, origin):
    lat_lon = request.get_json(force=True)
    task = detections.apply_async(args=[cur, models, lat_lon, date, pollutant, metric, origin])
    return task
