...
def init_weather_conv(cp, dataset):
    ...
    network2 = lasagne.layers.Conv2DLayer(incoming=network2,
                                         num_filters=int(cp.get('W2', 'convfilters')),
                                          filter_size=(
                                             int(cp.get('W2', 'filtersize')), int(cp.get('W1', 'filtersize'))),
...
def init(cp, dataset):
    ...
    test = network = lasagne.layers.DenseLayer(incoming=network,
                                        num_units=int(
                                            cp.get('NeuralNetwork', 'dense0')),
                                        )
    network = lasagne.layers.DenseLayer(incoming=network,
                                        num_units=int(
                                            cp.get('NeuralNetwork', 'dense1')),
                                        )
    network = lasagne.layers.ReshapeLayer(incoming=network,shape=([0],1,cp.getint('NeuralNetwork','featurex'),cp.getint('NeuralNetwork','featurey')))
    deep1 = lasagne.layers.Conv2DLayer(incoming=network,
                                         num_filters=def_filters, filter_size=(
                                             def_filter_size, def_filter_size),
                                         stride=def_stride,
                                         pad=def_padding
                                         )
    deep1 = lasagne.layers.Conv2DLayer(incoming=deep1,
                                         num_filters=int(cp.get('Deep1', 'convfilters')), filter_size=(
                                             int(cp.get('Deep1', 'filtersize')), int(cp.get('D1', 'filtersize'))),
                                         stride=int(cp.get('Deep1', 'stride')),
                                         pad=int(cp.get('Deep1','pad')))
    deep1 = lasagne.layers.MaxPool2DLayer(
        incoming=deep1, pool_size=(int(cp.get('Deep1','pool')), int(cp.get('Deep1','pool'))))
    deep1 = lasagne.layers.FlattenLayer(incoming=deep1)
    deep2 = lasagne.layers.Conv2DLayer(incoming=network,
                                         num_filters=def_filters, filter_size=(
                                             def_filter_size, def_filter_size),
                                         stride=def_stride,
                                         pad=def_padding
                                         )
    deep2 = lasagne.layers.Conv2DLayer(incoming=deep2,
                                         num_filters=int(cp.get('Deep2', 'convfilters')), filter_size=(
                                             int(cp.get('Deep2', 'filtersize')), int(cp.get('D1', 'filtersize'))),
                                         stride=int(cp.get('Deep2', 'stride')),
                                         pad=int(cp.get('Deep2','pad')))
    deep2 = lasagne.layers.MaxPool2DLayer(
        incoming=deep2, pool_size=(int(cp.get('Deep2','pool')), int(cp.get('Deep2','pool'))))
    deep2 = lasagne.layers.FlattenLayer(incoming=deep2)
    network = lasagne.layers.ConcatLayer(incomings=(deep1,deep2))
    network = lasagne.layers.DenseLayer(incoming=network,
                                        num_units=int(
                                            cp.get('NeuralNetwork', 'dense2')),
                                        )
    network = lasagne.layers.DenseLayer(incoming=network,
                                        num_units=int(
                                            cp.get('NeuralNetwork', 'dense3')),
                                        )
    network = lasagne.layers.DenseLayer(incoming=network,
                                        num_units=int(
                                            cp.get('NeuralNetwork', 'dense4')),
                                        )
    # Softmax
    network = lasagne.layers.DenseLayer(incoming=network,
                                        num_units=int(
