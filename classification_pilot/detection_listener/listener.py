...
def cdetections(date,pollutant,metri,origin):
    ...
    for m in models:
        if origin == m[0]:
            ...
            cl = m.get_output(items,det_map).argmax(axis=1)[0]
    ...
    for row in res:
        ...
        disp_results.append(row[0],1-scipy.spatial.distance.cosine(det,det_map))
    ...
    for row in res:
        if 'mult' in origin:
            ...
            citems = citems[:,1,:,:]
            citems = scale(citems)
        weather_results.append(row[0],1-scipy.spatial.distance.cosine(items,citems))
    results = []
